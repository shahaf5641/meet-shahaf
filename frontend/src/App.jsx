...
function Avatar({ state, analyserRef, mousePosRef }) {
  ...
  const { animations: idleAnims    } = useGLTF('/animations/Idle.glb')
  const { animations: talkAnims    } = useGLTF('/animations/Talking.glb')
  ...
  useEffect(() => {
    if (!hasWaved.current) return   // let wave finish first
    if (state === 'talking')       playAnim('talking',  0.3)
    else if (state === 'thinking') playAnim('thinking', 0.4)
    else                           playAnim('idle',     0.5)
  }, [state, actions])

  // Subtle mouse head-tracking on top of animation
  const mouseHeadOffset = useRef({ x: 0, y: 0 })
  useFrame(() => {
    if (!group.current) return
    const mx = mousePosRef?.current?.x || 0
    const my = mousePosRef?.current?.y || 0
    mouseHeadOffset.current.x += (my * 0.18 - mouseHeadOffset.current.x) * 0.04
    mouseHeadOffset.current.y += (mx * 0.30 - mouseHeadOffset.current.y) * 0.04
    ...
    group.current.traverse(obj => {
      ...
      if (nl === 'head' || nl === 'neck') {
        ...
        obj.rotation.x += mouseHeadOffset.current.x * s
        obj.rotation.y += mouseHeadOffset.current.y * s
      }
    })
  })
