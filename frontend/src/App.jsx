...
const STATIC_ANSWERS = {
  'ספר לי על ניסיון העבודה שלך ב-Hexagon': {
    ...
    transcript: "ב-Hexagon עשיתי internship כחלק מצוות הפיתוח, ועבדתי בצמוד ל-SDET של הצוות. עבדתי בסביבה אג'ילית עם C#, Azure DevOps ומערכות מבוססות אירועים — בניתי health-check tests, שיפרתי תשתיות בדיקות קיימות, ובניתי מאפס תשתית לבדיקות הרשאות end-to-end לפי תפקידי המערכת. הניסיון הזה לימד אותי לעבוד בצורה מסודרת בסביבה תעשייתית, לכתוב קוד איכותי, ולהשתלב נכון בתוך צוות פיתוח.",
  },
  ...
  async function startCall() {
    ...
    try {
      ...
      await new Promise((resolve) => {
        ...
        setCallState('active')
        setTranscript('היי, נעים מאוד, אני שחף ישראל, בן עשרים ושמונה מקריית אתא, בוגר הנדסת תוכנה ממכללת אורט בראודה. במהלך הלימודים עשיתי התמחות בחברת Hexagon, שם עבדתי בצוות פיתוח ועבדתי בעיקר על תשתיות בדיקות ואוטומציה. בין הדברים שעשיתי שם: יצרתי בדיקות health-check למערכת מבוססת אירועים, שיפרתי את תשתית יצירת האובייקטים בטסטים, ובניתי מאפס תשתית לבדיקות הרשאות מקצה לקצה לפי תפקידי המערכת. מעבר לזה, פיתחתי כמה פרויקטים עצמאיים — ביניהם EscapeCode, משחק פאזלים תלת-ממדי ללימוד קוד עם דגש על נגישות, ופרויקט אוטומציה לאיסוף נתונים מקבוצות פייסבוק עם ממשק web מלא. היום אני מחפש את התפקיד הבא שלי — מקום שבו אוכל לצמוח, להתפתח, ולהביא ערך אמיתי.')
