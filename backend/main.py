...
- לגבי Java: יש ניסיון בסיסי בלבד מפרויקט תואר (JavaFX). C# זו השפה החזקה, לא Java.
- לגבי Docker: ניסיון בסיסי — הרצת קונטיינרים ו-docker-compose. לא ניסיון עמוק.
- לגבי SQL: ידע בסיסי — שאילתות, JOINs, PostgreSQL ב-Facebook Data Extractor. לא מומחה.
- לגבי Azure: בעיקר Azure DevOps (Pipelines, CI/CD) ב-Hexagon. לא ניסיון עמוק ב-Azure Cloud Services.
